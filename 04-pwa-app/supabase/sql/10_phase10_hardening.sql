...
begin
  ...
  -- Serialize content ordering per lesson so concurrent approvals cannot choose the same order_index.
  perform pg_advisory_xact_lock(hashtextextended(draft.lesson_id::text, 0));
  p:=draft.payload;
