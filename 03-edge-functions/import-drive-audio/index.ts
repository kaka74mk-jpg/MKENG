...
const CORS_HEADERS = {
  // The function is authenticated by Supabase JWT. If you later deploy the PWA
  // on a fixed domain, replace * with that exact origin.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Resolve the source through the authenticated user's ownership before doing
    // any network work. Never trust a client-supplied lesson/source relationship.
    const { data: source, error: sourceError } = await adminClient
      .from("audio_sources")
      .select("id, user_id, lesson_id, source_type")
      .eq("id", audio_source_id)
      .eq("user_id", userId)
      .maybeSingle();

    if (sourceError || !source) {
      return jsonResponse({ error: "Audio source not found for this user" }, 404);
    }
    if (source.lesson_id !== lesson_id || source.source_type !== "google_drive_import") {
      return jsonResponse({ error: "Audio source does not match the requested lesson/import type" }, 400);
    }
    ...
    if (format === "unknown") {
      await markError(adminClient, audio_source_id, userId, "فرمت فایل صوتی پشتیبانی نمی‌شود.");
      return jsonResponse({ error: "Unsupported audio content type" }, 415);
    }
    if (fileBytes.byteLength > 200 * 1024 * 1024) {
      await markError(adminClient, audio_source_id, userId, "حجم فایل صوتی بیشتر از 200MB است.");
      return jsonResponse({ error: "Audio file exceeds the 200MB limit" }, 413);
    }
